...
const Content = () => {
    const [currentInView, setCurrentInView] = useState(null); // State to track only one div
    const divRefs = useRef([]); // Refs for each div
    useEffect(() => {
        const observer = new IntersectionObserver(
            ...
            { threshold: 0.5 } // Adjust the threshold for desired visibility
        );

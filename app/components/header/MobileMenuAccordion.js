        ...
        <li className='my-3 first:mt-0 last:mb-0'>
            <span className='flex items-center justify-between pb-3 cursor-pointer' onClick={handleToggle}>
                ...
                </span>
                {
                    data && data.children && (
                        <div>
                            <svg className={`${toggleOpen ? 'rotate-180' : 'rotate-0'} transition-all duration-300 ease-in-out`} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {data.children.map((child, index) => {
                            return (
                                <li key={index} className='py-1 first:pt-4 last:pb-4'>
                                    {child.url && (
                                        <Link {...(child?.noIndex ? { rel: 'nofollow' } : {})} onClick={handleMobileMenuToggle} href={child.url}>{child.title}</Link>
                                    )}
                                    {!child.url && (
                                        child.title
                                    )}

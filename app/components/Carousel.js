            ...
            <div className='absolute bottom-[20px] md:bottom-[50px] w-full overflow-auto pb-4'>
                <div className='container justify-left flex gap-5 md:justify-center'>
                    {data && data?.filters.map((filter, index2) => {
                        ...
                        return (
                            <div onClick={() => handleSelect(index2)} key={index2} className={`whitespace-nowrap mm-button default hover:bg-[#2657D4] hover:border-[#2657D4] cursor-pointer ${buttonClasses} relative overflow-hidden z-1`}>
                                <div className={`bg-[#1a3e96] w-0 opacity-80 absolute h-full top-0 left-0 duration-[${duration}s] ease-linear ${widthClass}`}></div>
